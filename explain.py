...
import dataTypes
# from old_flow_of_control import last_sequence_elements
import outFile
...
def move_to_inhibited_list(the_group):
    ...
    for group_conn in the_group.groups:
    ...
    if not the_group in the_analog.inhibited:
        ...
        # DIAG
        if dataTypes.diagnostics:
            LISA_gui.status_window.add_message(popped.name+' has been popped off '+the_analog.name+'s group list')
...
def add_group_to_sequence(driver, recip, group, goal_list, immediate_goal, random_firing_order=False):
    ...
    if random_firing_order:
        ...
        for i in xrange(int(prop_count*1.5)): # give it extra chances to fire props at random to cover all
            ...
            dataTypes.last_sequence_elements.append(current_sequence.index)
    else:
...
def make_new_effect_groups(target_group):
    ...
    for prop in target_group.prop:
        ...
        if dataTypes.diagnostics:
            ...
            print 'Sub-groups:',
            for sub_group in new_group.groups:
...
##            # DIAG
##            print
##            print ''.join(['Adding group ',group.name,' in the Workspace to the Sequence as driver.'])
##            print
##        else:
##            print
##            print ''.join(['NOT adding group ',group.name,' in the Workspace to the Sequence as driver because it has level ',str(group.level)])
##            print
##    else: # on the for
##        print
##        print ''.join(['* * * No groups in the Workspace to make the driver. * * *'])
##        print
##        
##        
##            # end DIAG
...
def return_control_to_explanandum(current_sequence, simulation, batch_run):
    ...
    if dataTypes.explanandum.unexplained_effects:
        ...
        if batch_run:
            return [True, chosen_group] # [False, chosen_group] # False means "don't quit"
        else:
...
def retrieve_prop_from_LTM(simulation): ## HERE 10/7/13: Prop retrieval happening TOO early! Look at this algorithm. Make it wait longer.
...
def retrieve_group_from_LTM(simulation):
    ...
    #
    # make a list of candidate groups consisting of all groups in dormant analogs
    # then from that candidate list retrieve using the Luce Coice axiom
    candidate_groups = []
    for analog in simulation['dormant']:
    ...
    if retrieved_group:
        ...
        if retrieved_group.type in ['CAUSE','EFFECT','ENABLER','ENABLED']:
            # 1) try to just grab its CE group
            try:
                retrieved_group = retrieved_group.parent_groups[0].unit
                done = True
            except AttributeError as exception_type : # if that fails, search through the list
                ...
                for group in candidate_groups:
                    ...
                    if group.type in ['CAUSAL','ENABLING']:
                        ...
                        for group_conn in group.groups:
...
def group_already_in_workspace(target_group):
    ...
    result = None
    for group in dataTypes.workspace.group:
        if group.name == target_group.name:
            result = group
            break
    return result
...
def copy_group_into_workspace(group, parent_group=None, cause_group=None, effect_group=None):
    # 4/15/08 for vers ex 1.08: Modified 4/5/14
...
##        parent_name = 'None'
##    if cause_group:
##        cause_name = cause_group.name
##    else:
##        cause_name = 'None'
##    if effect_group:
##        effect_name = parent_group.name
##    else:
##        effect_name = 'None'
##    print ''.join(['copying ',group.name,' into Workspace: Parent = ',parent_name,';  Cause = ',cause_name,';  Effect = ',effect_name])
##    # end DIAG

    # 1) copy in the group, naming and typing it after group
    if type(group) == dataTypes.CEGroup:
        # copy the entire structure, CE, Cause and Effect, into the workspace
        # 1.1) first, the cause
        cause_group = group_already_in_workspace(group.cause_group.unit)
        # now create the group if it doesn't exist already
        if not(cause_group):
            cause_group = copy_group_into_workspace(group.cause_group.unit, group) # copy into workspace, indicating that this group is the parent
        # 1.2 next, the effect
        effect_group = group_already_in_workspace(group.effect_group.unit) # copy into workspace, indicating that this group is the parent
        # now create the group if it doesn't exist already
        if not(effect_group):
            effect_group = copy_group_into_workspace(group.effect_group.unit, group)
        # 1.3 finally, the CE group
        if not(group_already_in_workspace(group)):
            workspace.group.append(dataTypes.CEGroup(group.name, workspace, cause_group, effect_group, group.causal_power, group.truth_status))
            # args: my_name, my_analog, cause_group, effect_group, causal_power, truth_status
            # set the level of the CE group to 1
            workspace.group[-1].level = 1

        # 1.4 Add the cause_group and the effect_group as a cause (child) and effect (child) to the CE group
            # not necessary: somehow, this is already being done (step 5 below)
        workspace.group[-1].add_group(cause_group)
        workspace.group[-1].add_group(effect_group)
        
    elif type(group) == dataTypes.CauseGroup:
        if not(group_already_in_workspace(group)):
            workspace.group.append(dataTypes.CauseGroup(group.name, workspace, [], group.truth_status))
            # args: my_name, my_analog, props = [], truth_status=None
    elif type (group) == dataTypes.EffectGroup:
        if not(group_already_in_workspace(group)):
            workspace.group.append(dataTypes.EffectGroup(group.name, workspace, [], group.truth_status))
        # args: my_name, my_analog, props = [], truth_status=None
    else:
...
##    if parent_group:
##        parent_group.groups.append(dataTypes.GroupConnector(new_group))

    # 6) return the group you just created
    return new_group
...
def show_contents_of_workspace():
    ...
    for group in dataTypes.workspace.group:
        print
        print '----------------'
        print ''.join(['Group ',group.name,' Level ',str(group.level)])
        print str(group.num_groups)+' Subgroups:',
        for gr_conn in group.groups:
...
##            # DIAG
##            print
##            print 'About to copy groups into workspace. Here is my list:'
