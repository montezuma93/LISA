# dataTypes.py
#
# Vers Ex 2.01, 11/22/13: Changing group data structures to allow more intelligent metacognition for schema induction
# Vers Ex 2.00, 11/21/13: SP BU (Hebb, too?) input: r(r+f), where r = role input; f = filler input
# Vers Ex 1.17: Add lateral inhibition to units in LTM during retrieval (10/24/13)

last_modified = '4/15/14'
version_id    = 'Ex 2.01'
...
def_scr_width =  1500 # 1200 # 1500 # default screen width
def_scr_height =  1000 # 750 # 900 # default screen height
...
pretty_format = False # pretty format puts props centered above SPs and groups centered over props
...
go_path  =  'lovetri/' # 'minister/' #'inductions/typicality3/' # typicality2/' # 'inductions/diversity2/' # 'semantic-2/' # 
go_file_name =  'lovetri' # 'hard4' #'RobinsHaveEnzyme' # 'birdlung' # 'SimilarSchema' # 'test1' # 

data_directory   = 'DATA/' # obviated 10/14/09 w/ new file-readin routines
# file_path        = ''    # directory path, within Data/, for .sym and .run file
current_directory = ''.join([os.getcwd(),'/']) # current directory will be '/Users/john/PyLISA/VersEx1.15/'
# now, rather than just a string, make the path a list of strings, where each string is either
#   the current directory or one directory below that
file_path        = current_directory+'Data/'   # path qua string, as used by old (pre vers Ex 1.15) LISA
path_list        = [current_directory,'Data/'] # path qua list for new (vers Ex 1.15) LISA
file_name        = '' # just the file name
sym_file_name    = '' # is gonna be path+file name, all in one string
out_file_name    = ''
...
driver_inhibition  = 1.0      # full by default I may have fucked this up -Zach
recip_inhibition   = 0.5  # 2/13/15     1.0      # full by default
dormant_inhibition = 0.5      # full by default -- vers Ex 1.17, 10/24/13
...
op_to_op_inhib        = -1.0 # 0.0  # by default, no inhib, but for some purposes useful
out_prop_prop_to_sp   = 0 # -0.5 2/13/15      -1.0  # -0.5  # -1.0
default_out_prop_to_sp = 0 # -0.5 2/13/15    -1.0
out_prop_sp_to_pred   = 0 # -0.5 2/13/15     -1.0  # -0.5  # -1
default_out_sp_to_pred = 0 # -0.5 2/13/15  -1.0
out_prop_sp_to_obj    = 0 # -0.5 2/13/15    -1.0  # -0.5  # -1
default_out_sp_to_obj = 0 # -0.5 2/13/15     -1.0
pred_to_sp            = 1.0   # 1.5
obj_to_sp             = 1.0   # 0.5
sem_to_pred           = 1.0   # 1.5
sem_to_obj            = 1.0   # 0.5
...
retrieval_threshold   = 0.15 # 0.35 # 0.4   # activation for recip unit to get retrieved into WM
...
class HebbUnit(object):  # this is the unit shared by the two hebb sets
    def __init__(self, owner1, owner2):
        self.weight = 0.0
        self.delta = 0.0 # 12/16/13, Vers Ex 2.01, most recent change (will be zero if no recent change) -- YO! This Code Yet To Be Written! (12/16/13)
        self.act = 0.0
        self.input = 0.0
        self.buffer = 0.0
        self.normalizer = 1.0
...
class HebbSet(object):
    ...
    def add_hebb(self, token_unit, h_unit):
        ...
        # now return the return val (either the new hebb or None)
        return return_val
    ...
    def get_max_hebb_unit(self):
        # 12/16/13, vers Ex 2.01
        # returns a pointer to the maximum hebb in the set: returns the pointer to the whole hebb, unit, weight and all
        max_hebb   = None
        if self.hebbs:
            max_hebb = self.hebbs[0] # start at beginning
            for hebb in self.hebbs:
                if hebb.h_unit.weight > max_hebb.h_unit.weight:
                    max_hebb = hebb
        return max_hebb
        
    def get_uniqueness(self, candidate_max = None):
        # 12/16/13, vers Ex 2.01
        # returns the uniquenes of the maximum mapping, that is, the max_mapping minus the second max

        # if candidate_max is not None -- i.e., if it is a Hebb (possiby the max_Hebb) -- then get_uniqueness returns 0.0 if there is any
        #   hebb with a greater weight, or else returns the greater of 0 or (candidate_max.h_unit.weight - second_max)
        max_weight = 0.0
        second_max = 0.0
        for hebb in self.hebbs:
            if hebb.h_unit.weight >= max_weight:
                second_max = max_weight
                max_weight = hebb.h_unit.weight
            elif hebb.h_unit.weight >= second_max:
                second_max = hebb.h_unit.weight

        if candidate_max == None:
            return max_weight - second_max
        else:
            uniqueness = candidate_max.h_unit.weight - second_max
            if uniqueness < 0.0: uniqueness = 0.0
            return uniqueness
...
class Unit(object):
    ...
    def show(self, message = ''):
        # a fully disgnostic method (12/18/13)
        print message,'Unit = ',self.name,'Type =',str(type(self))
...
ConfidenceChangeRate = 0.5 # like a learning rate: How rapidly Inductive Confidence chnges in response to influence from other units

class InductiveConfidence(object):
    '''
    Inductive Confidence class, for vers Ex 2.01, 12/16/13

    This is the property of proposition units and CE groups

    Value is proportional to the belief that the corresponding assertion (proposition or Cause-Effect relation) is true:
        -1.0 = Complete confidence that asertion is false
        0.0 = complete and utter uncertainty about truth
        1.0 = complete certinty that ssertion is true

    Owner is the unit (prop or CE group) to which the inductive confidence corresponds

    '''
    def __init__(self, owner, initial_value = 0.5):
        self.owner = owner
        self.value = initial_value # see above: Value ranges -1...1; set initial value to 0.5 by default
        self.delta_value = 0.0 # change in value: When updating, compute all changes first, then update values (synchronous updating)

        # DIAG
        self.value_history = [self.value] # a list of all values

    def get_delta_value(self):
        # updates value of inductive confidence according to:
        # (a) strengths of mapping(s) to other units, j
        # (b) immediate changes in the values of those mappings
        # (c) uniqueness of those mappings
        # (d) existing inductive confidence of the units on the other ends of those mappings
        # (e) own existing inductive confidence

        # calculate the evidence for change (terms (a)...(d) above): This is the real meat
        # 0) Init evidence
        evidence = 0.0 # evidence corresponds to terms (a)...(d) above
        self.delta_value = 0.0 # init delta to zero and increment separately for every hebb set
        # 1) go through all the hebb sets...
        for hebb_set in self.owner.hebb_set:
            max_hebb      = hebb_set.get_max_hebb_unit()
            if max_hebb: # only engage in the following if there IS a max_hebb...
                weight        = max_hebb.h_unit.weight # (a) above
                recent_change = max_hebb.h_unit.delta # (b) above 

                # HERE 12/18/13: for making sure update happens for recently inferred units
                if self.owner.is_infant:
                    recent_change = 1.0
                    
                uniqueness    = hebb_set.get_uniqueness(max_hebb)  # (c) above
                try:
                    confidence    = max_hebb.recip.inductive_confidence.value # (d) above
                except:
                    max_hebb.recip.show('ERROR: No inductive_confidence on ')
                    confidence = 0.0
                evidence      = weight * recent_change * uniqueness * confidence
            
                # calculate delta_value based on evidence, change rate and current value
                self.delta_value += ConfidenceChangeRate * evidence * (1.0 - abs(self.value)) # the (1.0 - ...) is term (e) above

    def update_value(self):
        # updates value of inductive confidence based on delta value
    ...
    def print_history(self):
        # DIAG print unit's history of inductive confidence
        print
        if type(self.owner) == PropUnit:
            print self.owner.pretty_name
        else:
...
class PropUnit(TokenUnit):
    def __init__(self, my_name, my_analog):
        ...
        self.inductive_confidence = InductiveConfidence(self) # new vers 2.01, 12/06/13 (like probability)
    ...
    def get_bu_input(self):
        # new vers Ex 2.00, 11/21/13

        self.bu_input = 0.0
        for SP in self.SPs:
            self.bu_input += SP.output # act
...
class SPUnit(TokenUnit):
    ...
    def get_bu_input(self):
        ...
        # get input from filler (obj or child prop)
        filler_input = 0.0
        if self.child_prop:
            if self.child_prop.mode == -1: # child mode
                filler_input = self.child_prop.act
        elif self.obj:
            filler_input = self.obj.act

        # get input from role
        if self.pred: role_input = self.pred.act
        else: role_input = 0.0

        # set bu_input (rule of 11/21/13: role necessary PLUS benefit from matching filler
        self.bu_input = role_input * (pred_to_sp * role_input + obj_to_sp * filler_input)


    def update_activation(self, gamma, delta):
        # New, vers Ex 2.00, 11/21/13: Input is a complex AND of role and filler inputs
...
class OPUnit(TokenUnit):
    ...
    def get_bu_input(self):
        # new vers Ex 2.00, 11/21/13
        self.bu_input = 0.0
        for sem_conn in self.semantic: # list of units connections to semantics
            self.bu_input += sem_conn.weight * sem_conn.unit.act
        # normalize by Weber Law
        self.bu_input /= self.weber_sum           

    def add_semantic(self, sem_unit, wt):
        self.semantic.append(Link(sem_unit, wt))
        self.weber_sum += wt

    def compute_weight_length(self):  # for the cosine input function & similarity
...
class GroupConnector(Link):     # this is a connection between groups
    def __init__(self, unit, wt=1.0):
        Link.__init__(self, unit, wt)
...
class GroupUnit(TokenUnit):
    def __init__(self, my_name, my_analog, my_type='GENERIC', truth_status=None):
        ...
        # other group units Below this group
        self.groups = [] # was self.group = [] (12/18/13)
        self.num_groups = 0
        
        # groups Above this group (for now, 3/2/11) this is limited to just one parent... I may have to relax this later
        self.parent_groups = []
        ...
        self.inductive_confidence = InductiveConfidence(self) # new vers 2.01, Added to generic group type on 4/6/14
    ...
    def add_group(self, child, wt=1.0):
        # adds a group to self as a Child (in self.groups) and adds self to the Child's list of parents
        # also helps CE, Cause and Effect groups keep track of their relationships vis. Cause and Effect groups
        # 0) before you begin, make sure the potential child isn't already in there
        already_there = False
        for existing in self.groups:
            if child == existing.unit:
                already_there = True
                break
        if not(already_there):
            # 1) Add the Child to this group's list of groups
            self.groups.append(GroupConnector(child, wt))
            self.num_groups += 1
            # 2) Add self to the child's list of parents
            child.parent_groups.append(GroupConnector(self, wt))
            # If self is a CE group, then also add the child to the appropriate Cause and Effect fields
            if type(self) == CEGroup:
                if type(child) == CauseGroup:
                    self.cause_group = self.groups[-1] # the last-addded child group is also the CauseGroup
                elif type(child) == EffectGroup:
                    self.effect_group = self.groups[-1]

    def add_prop(self, prop_unit, wt=1.0):
        self.prop.append(GroupConnector(prop_unit, wt)) # data type changed from Link to Group connector (to add integrator) 12/18/13
        self.num_props += 1

    def add_semantic(self, sem_unit, wt=1.0):
        self.semantic.append(Link(sem_unit, wt))
        self.weber_sum += wt
...
## cause-effect groups (vers Ex 2.00, 11/22/13)

class CEGroup(GroupUnit):
    def __init__(self, my_name, my_analog, cause_group, effect_group, causal_power, truth_status=None):
        GroupUnit.__init__(self, my_name, my_analog, 'CE', truth_status)

        # CE-group-specific structures
        self.cause_group = None
        self.effect_group = None
        if cause_group: self.add_group(cause_group)
        if effect_group: self.add_group(effect_group)
        
        self.level        = 1 # level 1 (one above 0; 0 is te level of Cause and Effect) by default

        # also add the same cause and effect groups to the CE group's more generic groups list
        # This is both useful and dangerous because it makes it possible to access the same data structures in multiple ways
##        self.groups.append(self.cause_group) # note that we're not using the add method here 'cause the Connector already exists; we're just pointing to it
##        self.groups.append(self.effect_group) # note that we're not using the add method here 'cause the Connector already exists; we're just pointing to it

##        # and add the CE group to the cause and effect groups as parent
##        if cause_group:  cause_group.parent = GroupConnector(self)
##        if effect_group: effect_group.parent = GroupConnector(self)

        self.causal_power = causal_power # 0...1; power in a PPC sense; will affect retrieval and also connections to power semantics

        # self.inductive_confidence = InductiveConfidence(self) # new vers 2.01, 12/16/13 (like probability) -- replaces truth_status
        # inductive confidence moved to generic group type on 4/6/14


class CauseGroup(GroupUnit):
    def __init__(self, my_name, my_analog, props = [], truth_status=None):
        GroupUnit.__init__(self, my_name, my_analog, 'CAUSE', truth_status)

        # Cause-group-specific structures (none, really)

        # add props passsed in, if any
        for prop in props:
            self.add_prop(prop, 1.0)

class EffectGroup(GroupUnit):
    def __init__(self, my_name, my_analog, props = [], truth_status=None):
        GroupUnit.__init__(self, my_name, my_analog, 'EFFECT', truth_status)

        # Effect-group-specific structures (none, really)
...
### end specific cause-related group units
            ...
            for group in self.groups:
                if group.level > 0:
                    ...
                    if group.groups:
                        mean_x = 0
                        n      = 0
                        for group_link in group.groups:
                            mean_x += group_link.unit.rect[0]
                            n      += 1
                        if n > 0:
                            mean_x = int(round(float(mean_x/n)))
                            group.rect[0] = mean_x
                            group.act_rect[0] = mean_x + 1
            # update analog width
            for group in self.groups:
                if (group.rect[0] + unit_width) > width: width = self.groups[-1].rect[0] + unit_width + 1
